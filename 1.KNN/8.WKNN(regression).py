# [MXML-1-06] 8.WKNN(regression).py
#
...
# Normalization
x_mu = x.mean(); x_sd = x.std()
y_mu = y.mean(); y_sd = y.std()
...
dist = np.sqrt(np.sum((d_train - d_test) ** 2, axis=2)) + 1e-8

# Find K-nearest neighbors, and calculate inverse weighted distance
K = 20
...
# Estimate the standardized y-values of the test data and
# restore them back to their original scale.
zy_pred = (y_near * w_inv).sum(axis=1) / w_inv.sum(axis=1)
y_pred1 = zy_pred * y_sd + y_mu
plot_prediction(x, y, x_test, y_pred1) # visualize the result
...
plot_prediction(x, y, x_test, y_pred2)  # visualize the result
